...
export default class PrivateMeetingSearch extends  Component {
    constructor() {
        super();
        this.state = {
            sessionsRaw: [],
    ...
    componentDidMount = async () => {
        try {
            const response = await fetch(url +'getPrivateSessions/').then()
            ...
            <View style = {{flex: 1, backgroundColor: "#faf8f8"}}>
                <View>
                    <Header style = {{backgroundColor: 'white', borderBottomWidth: 2, borderBottomColor: '#f18a21'}} >
                        ...
                        <Text style = {{marginTop: 10, fontSize: 20, fontFamily: "Helvetica-Bold"}}>Birebir Terapi</Text>
                            ...
                            <TouchableOpacity
                                onPress={() => {
                                    ...
                                    AsyncStorage.setItem("sessionIdForSessionDetail", item.sessionId + "").then(
                                        this.props.navigation.navigate('ÜcretliTerapiDetails')
                                    )
                                }}
                            >
